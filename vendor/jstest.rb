...
class TestBuilder
  ...
  FIXTURES_DIR       = File.join(UNITTEST_DIR, 'unit')
  
  def initialize(file_name)
    @file_name    = file_name
    @js_file_name = File.basename(@file_name)
    @basename     = @js_file_name.sub("_test.js", "")
    @title        = @basename.gsub("_", " ").strip.capitalize
  end
  
  def find_fixtures
    @fixtures = ""
    file = File.join(FIXTURES_DIR, "#{@basename}.#{FIXTURES_EXTENSION}")
    if File.exists?(file)
  ...
  def destination
    basename = File.basename(@file_name, ".js")
    File.join(UNITTEST_DIR, 'unit', "#{basename}.html")
  end
end
